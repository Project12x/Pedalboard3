...
- **Scaled footer layout** now promotes the main footer into compact or stacked arrangements using scale-aware available width, so core controls remain reachable instead of being clipped or hidden at high Pedalboard UI scale.
- **Preferences high-scale layout** now moves UI Scale into the first visible block and uses a vertical viewport so lower settings remain reachable when the dialog is narrow at 150% and 200% app scale.
- **D2 visual QA script** now accepts `-UiScalePercent`, records `uiScalePercent` in capture summaries, adds `-CaptureScaledFooterMatrix` for normal and narrow footer evidence at 125%, 150%, 175%, and 200%, and adds `-CaptureScaledDialogMatrix` for plugin search, Preferences, NAM browser, and IR browser at 150% and 200%; the prior exact display-scale guard is retained only as `-ExpectedOsScalePercent` for OS-DPI compatibility checks.
- **QA docs** updated to require app-scale captures at 75%, 100%, 125%, 150%, 175%, and 200%, without asking testers to change Windows Display scale.
