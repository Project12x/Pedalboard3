...
class LADSPAModuleHandle : public ReferenceCountedObject
{
  public:
    ...
    //==============================================================================
    LADSPAModuleHandle(const File& file_) : file(file_), moduleMain(0), hModule(0)
    {
    ...
    bool open()
    {
        ...
        {
            moduleMain =
                (LADSPA_Descriptor_Function)PlatformUtilities::getProcedureEntryPoint(hModule, "ladspa_descriptor");
        }
...
LADSPAPluginInstance::LADSPAPluginInstance(const ReferenceCountedObjectPtr<LADSPAModuleHandle>& module_)
    : ptrPlug(0), effect(0), initialised(false), isPowerOn(false), tempBuffer(1, 1), channels(0), module(module_)
{
    try
    {
        _fpreset();
...
{
    {
        ...
        if (unscaledParameters)
            delete[] unscaledParameters;
        if (normalParameters)
            delete[] normalParameters;
...
void LADSPAPluginInstance::initialise()
{
    ...
    setPlayConfigDetails(inputs.size(), outputs.size(), getSampleRate() ? getSampleRate() : 44100.0f,
...
//==============================================================================
void LADSPAPluginInstance::prepareToPlay(double sampleRate_, int samplesPerBlockExpected)
{
    setPlayConfigDetails(inputs.size(), outputs.size(), sampleRate_, samplesPerBlockExpected);
...
void LADSPAPluginInstance::processBlock(AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
...
{
    return (ptrPlug && ptrPlug->Label) ? String(ptrPlug->Label) : module->file.getFileNameWithoutExtension();
}
    ...
    if (ptrPlug != 0 && ((unsigned int)index) < (unsigned int)parameters.size())
    {
        try
        {
            ...
            if (unscaledParameters[index] != newValue)
            {
                ...
                float lower =
                    hint->LowerBound * (LADSPA_IS_HINT_SAMPLE_RATE(hint->HintDescriptor) ? getSampleRate() : 1.0f);
                float upper =
                    hint->UpperBound * (LADSPA_IS_HINT_SAMPLE_RATE(hint->HintDescriptor) ? getSampleRate() : 1.0f);

                // NOTE: LADSPA bound handling is simplified; edge cases may not be perfect
                ...
                {
                    if (LADSPA_IS_HINT_LOGARITHMIC(hint->HintDescriptor) && lower > 0.0f && upper > 0.0f)
...
#if JUCE_WIN32
    return f.existsAsFile() && f.hasFileExtension(T(".dl"));
#elif JUCE_MAC
    return f.existsAsFile() && f.hasFileExtension(T(".so"));
#elif JUCE_LINUX
    return f.existsAsFile() && f.hasFileExtension(T(".so")) && f.getFileNameWithoutExtension() != T("dssi-vst");
...
const StringArray LADSPAPluginFormat::searchPathsForPlugins(const FileSearchPath& directoriesToSearch,
