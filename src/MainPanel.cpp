...
public:
  PluginListWindow(KnownPluginList &knownPluginList, MainPanel *p)
    ...
    setContentOwned(new PluginListComponent(
    ...
    restoreWindowStateFromString(
        PropertiesSingleton::getInstance().getUserSettings()->getValue(
            "listWindowPos"));
    setVisible(true);
  }
  ...
  ~PluginListWindow() {
    ...
    PropertiesSingleton::getInstance().getUserSettings()->setValue(
        "listWindowPos", getWindowStateAsString());
  }
...
MainPanel::MainPanel(ApplicationCommandManager *appManager)
    : FileBasedDocument(".pd", "*.pd", "Choose a set of patches to open...",
                        "Choose a set of patches to save as..."),
      Thread("OSC Thread"), commandManager(appManager), currentPatch(0),
      patchLabel(0), prevPatch(0), nextPatch(0), patchComboBox(0), viewport(0),
      cpuSlider(0), cpuLabel(0), playButton(0), rtzButton(0), tempoLabel(0),
  ...
  addAndMakeVisible(playButton = new DrawableButton(
                        "playButton", DrawableButton::ImageOnButtonBackground));
  playButton->setName("playButton");

  addAndMakeVisible(rtzButton = new DrawableButton(
                        "rtzButton", DrawableButton::ImageOnButtonBackground));
  ...
  addAndMakeVisible(tapTempoButton = new ArrowButton("tapTempoButton", 0.0,
                                                     Colour(0x40000000)));
  ...
  cpuSlider->setColour(Slider::thumbColourId,
                       ColourScheme::getInstance().colours["CPU Meter Colour"]);
  ...
  std::unique_ptr<Drawable> rtzImage(
      loadSVGFromMemory(Vectors::rtzbutton_svg, Vectors::rtzbutton_svgSize));
  ...
  auto savedAudioState = // JUCE 8: returns unique_ptr
      PropertiesSingleton::getInstance().getUserSettings()->getXmlValue(
          "audioDeviceState");
  ...
  {
    auto midiDevices =
        MidiInput::getAvailableDevices(); // JUCE 8: Array<MidiDeviceInfo>
  ...
  PluginField *field =
      new PluginField(&signalPath, &pluginList, commandManager);
  ...
  // Setup the socket.
  {
    ...
    port = PropertiesSingleton::getInstance().getUserSettings()->getValue(
        "OSCPort", "5678");
    if (port == "")
      port = "5678";
    address = PropertiesSingleton::getInstance().getUserSettings()->getValue(
        "OSCMulticastAddress");

    if (PropertiesSingleton::getInstance().getUserSettings()->getBoolValue(
            "OscInput", "1")) {
  ...
  // Setup the program change warning.
  warningBox.reset(
      new CallOutBox(warningText, patchComboBox->getBounds(), this));
  ...
  {
    File defaultPatch =
        JuceHelperStuff::getAppDataFolder().getChildFile("default.pdl");
...
void MainPanel::resized() {
  ...
  playButton->setBounds(proportionOfWidth(0.5000f) - ((36) / 2),
                        getHeight() - 38, 36, 36);
  rtzButton->setBounds((proportionOfWidth(0.5000f) - ((36) / 2)) + 38,
                       getHeight() - 32, 24, 24);
  tempoLabel->setBounds((proportionOfWidth(0.5000f) - ((36) / 2)) + -151,
                        getHeight() - 33, 64, 24);
  tempoEditor->setBounds((proportionOfWidth(0.5000f) - ((36) / 2)) + -87,
                         getHeight() - 33, 52, 24);
  tapTempoButton->setBounds((proportionOfWidth(0.5000f) - ((36) / 2)) + -31,
                            getHeight() - 27, 10, 16);
...
void MainPanel::buttonClicked(Button *buttonThatWasClicked) {
  ...
  //[UserbuttonClicked_Post]
    ...
    PluginField *pluginField =
        dynamic_cast<PluginField *>(viewport->getViewedComponent());
...
void MainPanel::getCommandInfo(const CommandID commandID,
  ...
  switch (commandID) {
  case FileNew:
    result.setInfo("New", "Creates a new pedalboard file to work from.",
                   fileCategory, 0);
    result.addDefaultKeypress(L'n', ModifierKeys::commandModifier);
    break;
  case FileOpen:
    result.setInfo("Open...", "Opens an existing pedalboard file from disk.",
                   fileCategory, 0);
    result.addDefaultKeypress(L'o', ModifierKeys::commandModifier);
    break;
  case FileSave:
    result.setInfo("Save", "Saves the current pedalboard file to disk.",
                   fileCategory, 0);
  ...
  case EditDeleteConnection:
    result.setInfo("Delete selected connection(s)",
                   "Deletes the selected connection(s).", editCategory, 0);
  ...
  case EditOrganisePatches:
    result.setInfo("Organise patches", "Opens the patch organiser.",
                   editCategory, 0);
    break;
  case EditUserPresetManagement:
    result.setInfo("User Preset Management",
                   "Opens the user preset managemet window.", editCategory, 0);
    break;
  case OptionsPreferences:
    result.setInfo("Misc Settings", "Displays miscellaneous settings.",
                   optionsCategory, 0);
    break;
  case OptionsAudio:
    result.setInfo("Audio Settings", "Displays soundcard settings.",
                   optionsCategory, 0);
    break;
  case OptionsPluginList:
    result.setInfo("Plugin List", "Options to scan and remove plugins.",
                   optionsCategory, 0);
    break;
  case OptionsColourSchemes:
    result.setInfo("Colour Schemes", "Load and edit alternate colour schemes.",
                   optionsCategory, 0);
    break;
  case OptionsKeyMappings:
    result.setInfo("Application Mappings", "Change the application mappings.",
                   optionsCategory, 0);
    break;
  case HelpDocumentation:
    ...
    break;
  case HelpLog:
    result.setInfo("Event Log", "Displays an event log for the program.",
                   helpCategory, 0);
    break;
  case HelpAbout:
    result.setInfo("About", "Shows some details about the program.",
                   helpCategory, 0);
    break;
  case PatchNextPatch:
    result.setInfo("Next Patch", "Switches to the next patch.", patchCategory,
                   0);
    break;
  case PatchPrevPatch:
    result.setInfo("Previous Patch", "Switches to the previous patch.",
                   patchCategory, 0);
    break;
  case TransportPlay:
    result.setInfo("Play/Pause", "Plays/pauses the main transport.",
                   transportCategory, 0);
  ...
  case TransportTapTempo:
    result.setInfo("Tap Tempo", "Used to set the tempo by 'tapping'.",
                   transportCategory, 0);
    break;
  }
}
...
bool MainPanel::perform(const InvocationInfo &info) {
  ...
  case EditOrganisePatches:
    ...
    // Open the organiser.
    {
      ...
      JuceHelperStuff::showModalDialog(
          "Patch Organiser", &patchOrganiser, 0,
  ...
  case OptionsAudio: {
    AudioDeviceSelectorComponent win(deviceManager, 1, 16, 1, 16, true, false,
                                     false, false);
    win.setSize(380, 400);
    ...
    JuceHelperStuff::showModalDialog(
        "Audio Settings", &win, 0,
        ColourScheme::getInstance().colours["Window Background"], true, true);
    switchPatch(patchComboBox->getSelectedId() - 1, false, true);

    std::unique_ptr<XmlElement> audioState =
        deviceManager.createStateXml(); // JUCE 8: unique_ptr
    if (audioState) {
      PropertiesSingleton::getInstance().getUserSettings()->setValue(
          "audioDeviceState", audioState.get());
  ...
  case OptionsColourSchemes: {
    ...
    JuceHelperStuff::showNonModalDialog(
        "Colour Schemes", dlg, 0,
        ColourScheme::getInstance().colours["Window Background"], true, true);
  } break;
  case OptionsKeyMappings: {
    StupidWindow *win = (StupidWindow *)getParentComponent();
    ApplicationMappingsEditor editor(
        win->getAppManager(), field->getMidiManager(), field->getOscManager());

    editor.setSize(414, 524);
    JuceHelperStuff::showModalDialog(
        "Application Mappings", &editor, this,
  ...
  case PatchNextPatch:
    if (patchComboBox->getSelectedItemIndex() <
        (patchComboBox->getNumItems() - 2))
      patchComboBox->setSelectedItemIndex(
          patchComboBox->getSelectedItemIndex() + 1);
    else if (PropertiesSingleton::getInstance().getUserSettings()->getBoolValue(
                 "LoopPatches", true))
...
void MainPanel::changeListenerCallback(ChangeBroadcaster *changedObject) {
  ...
  if (changedObject == MainTransport::getInstance()) {
    ...
    if (savedPluginList != nullptr) {
      PropertiesSingleton::getInstance().getUserSettings()->setValue(
          "pluginList", savedPluginList.get());
...
void MainPanel::setSocketPort(const String &port) {
  ...
  PropertiesSingleton::getInstance().getUserSettings()->setValue("OSCPort",
                                                                 port);
}
...
void MainPanel::enableAudioInput(bool val) {
  ...
  PropertiesSingleton::getInstance().getUserSettings()->setValue("AudioInput",
                                                                 val);
}
...
void MainPanel::enableMidiInput(bool val) {
  ...
  PropertiesSingleton::getInstance().getUserSettings()->setValue("MidiInput",
                                                                 val);
}
...
void MainPanel::enableOscInput(bool val) {
  ...
  PropertiesSingleton::getInstance().getUserSettings()->setValue("OscInput",
                                                                 val);
}
...
//------------------------------------------------------------------------------
String MainPanel::getDocumentTitle() { return "Pedalboard2 Patch File"; }
...
Result MainPanel::loadDocument(const File &file) {
  ...
  if (root) {
    if (root->hasTagName("Pedalboard2PatchFile")) {
      ...
      // Clear patchComboBox.
      patchComboBox
          ->clear(); // JUCE 8: clear() takes no args (notification handled
        ...
        if (deviceXml) {
          ...
          if (err != "") {
            AlertWindow::showMessageBoxAsync(
                AlertWindow::WarningIcon, "Audio Device Error",
...
//------------------------------------------------------------------------------
File MainPanel::getLastDocumentOpened() { return lastDocument; }

//------------------------------------------------------------------------------
void MainPanel::setLastDocumentOpened(const File &file) { lastDocument = file; }
...
//------------------------------------------------------------------------------
void MainPanel::nextSwitchDoNotSavePrev() { doNotSaveNextPatch = true; }
...
Drawable *MainPanel::loadSVGFromMemory(const void *dataToInitialiseFrom,
  ...
  if (svgData != nullptr)
    retval = Drawable::createFromSVG(*svgData)
                 .release(); // JUCE 8: release ownership to match return type
