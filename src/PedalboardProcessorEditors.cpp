//	PedalboardProcessorEditors.cpp - The various editors for the app's
//internal 									 processors.
//	----------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
LevelControl::~LevelControl() { deleteAllChildren(); }
...
LevelEditor::LevelEditor(AudioProcessor *processor,
    ...
    : AudioProcessorEditor(processor), parentBounds(windowBounds),
      setPos(false) {
  slider = new Slider();
...
FilePlayerEditor::~FilePlayerEditor() {
  FilePlayerProcessor *proc =
      dynamic_cast<FilePlayerProcessor *>(getAudioProcessor());
...
OutputToggleControl::OutputToggleControl(OutputToggleProcessor *proc)
    : processor(proc) {
  ScopedPointer<Drawable> im1(JuceHelperStuff::loadSVGFromMemory(
      Vectors::outputtoggle1_svg, Vectors::outputtoggle1_svgSize));
  ScopedPointer<Drawable> im2(JuceHelperStuff::loadSVGFromMemory(
      Vectors::outputtoggle2_svg, Vectors::outputtoggle2_svgSize));

  toggleButton =
      new DrawableButton("toggleButton", DrawableButton::ImageFitted);
...
//------------------------------------------------------------------------------
OutputToggleControl::~OutputToggleControl() { deleteAllChildren(); }
...
void OutputToggleEditor::timerCallback() {
  toggleButton->setToggleState(getAudioProcessor()->getParameter(0) > 0.5f,
                               false);
...
void OutputToggleEditor::buttonClicked(Button *button) {
  getAudioProcessor()->setParameter(0, toggleButton->getToggleState() ? 1.0f
                                                                      : 0.0f);
}
...
VuMeterControl::VuMeterControl(VuMeterProcessor *proc)
    : processor(proc), levelLeft(0.0f), levelRight(0.0f) {
  startTimer(60);
...
void VuMeterControl::paint(Graphics &g) {
  ...
  const float twentyFourDb =
      redSize + ((24.0f / 60.0f) * (height - redSize - 4));
  const float fortyEightDb =
      redSize + ((48.0f / 60.0f) * (height - redSize - 4));
  map<String, Colour> &colours = ColourScheme::getInstance().colours;

  /*Colour topColour1((uint8)((255 * levLeft) + (0 * (1.0f-levLeft))),
                                    (uint8)((255 * levLeft) + (191 *
  (1.0f-levLeft))), (uint8)((0 * levLeft) + (0 * (1.0f-levLeft))), (uint8)(127 *
  levLeft)); Colour topColour2((uint8)((255 * levRight) + (0 *
  (1.0f-levRight))), (uint8)((255 * levRight) + (191 * (1.0f-levRight))),
                                    (uint8)((0 * levRight) + (0 *
  (1.0f-levRight))), (uint8)(127 * levRight));*/
  Colour topColour1 =
      colours["VU Meter Upper Colour"].withMultipliedBrightness(levLeft);
  Colour topColour2 =
      colours["VU Meter Upper Colour"].withMultipliedBrightness(levRight);
  ...
  if (levelLeft > -60.0f) {
    ...
    g.fillRect(0.0f, height - heightLeft - 4, (width * 0.5f) - 2.0f,
               heightLeft);
  }
  ...
  if (levelRight > -60.0f) {
    ...
    g.fillRect((width * 0.5f) + 2.0f, height - heightRight - 4,
               (width * 0.5f) - 2.0f, heightRight);
  }
  ...
  g.drawText("0dB", (int)((width / 2) - textSize), (int)(redSize - textSize),
             (int)(textSize * 2), (int)(textSize * 2),
             Justification(Justification::centred), false);
  g.drawText("6dB", (int)((width / 2) - textSize), (int)(sixDb - textSize),
             (int)(textSize * 2), (int)(textSize * 2),
             Justification(Justification::centred), false);
  g.drawText("12dB", (int)((width / 2) - (textSize * 2)),
             (int)(twelveDb - textSize), (int)(textSize * 4),
             (int)(textSize * 2), Justification(Justification::centred), false);
  g.drawText("24dB", (int)((width / 2) - (textSize * 2)),
             (int)(twentyFourDb - textSize), (int)(textSize * 4),
             (int)(textSize * 2), Justification(Justification::centred), false);
  g.drawText("48dB", (int)((width / 2) - (textSize * 2)),
             (int)(fortyEightDb - textSize), (int)(textSize * 4),
             (int)(textSize * 2), Justification(Justification::centred), false);
}
...
VuMeterEditor::~VuMeterEditor() {
  VuMeterProcessor *proc =
      dynamic_cast<VuMeterProcessor *>(getAudioProcessor());
...
AudioRecorderEditor::~AudioRecorderEditor() {
  RecorderProcessor *proc =
      dynamic_cast<RecorderProcessor *>(getAudioProcessor());
...
MetronomeEditor::~MetronomeEditor() {
  MetronomeProcessor *proc =
      dynamic_cast<MetronomeProcessor *>(getAudioProcessor());
