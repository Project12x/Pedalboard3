...
FilterConnection::FilterConnection(const FilterConnection &other)
    : sourceFilterID(other.sourceFilterID), sourceChannel(other.sourceChannel),
      destFilterID(other.destFilterID), destChannel(other.destChannel),
      owner(other.owner) {}
...
FilterGraph::FilterGraph()
    : FileBasedDocument(filenameSuffix, filenameWildcard, "Load a filter graph",
                        "Save a filter graph"),
      lastUID(0) {
  ...
  if (audioInput) {
    addFilter(internalFormat.getDescriptionFor(
...
FilterGraph::~FilterGraph() { graph.clear(); }

uint32 FilterGraph::getNextUID() throw() { return ++lastUID; }

//==============================================================================
int FilterGraph::getNumFilters() const throw() { return graph.getNumNodes(); }

const AudioProcessorGraph::Node::Ptr FilterGraph::getNode(const int index) const
    throw() {
  return graph.getNode(index);
}

const AudioProcessorGraph::Node::Ptr
FilterGraph::getNodeForId(const AudioProcessorGraph::NodeID uid) const throw() {
...
void FilterGraph::addFilter(const PluginDescription *desc, double x, double y) {
  if (desc != 0) {
    ...
    if (tempInstance) {
      if (dynamic_cast<AudioProcessorGraph::AudioGraphIOProcessor *>(
              tempInstance.get()) ||
...
void FilterGraph::setNodePosition(const int nodeId, double x, double y) {
  ...
  const AudioProcessorGraph::Node::Ptr n(
      graph.getNodeForId(AudioProcessorGraph::NodeID(nodeId)));
...
void FilterGraph::getNodePosition(const int nodeId, double &x,
                                  double &y) const {
  x = y = 0;
...
FilterGraph::getConnections() const {
  return graph.getConnections();
}

// JUCE 8: Implementation of getConnectionBetween - finds a matching connection
const AudioProcessorGraph::Connection *FilterGraph::getConnectionBetween(
  ...
  for (const auto &conn : connections) {
    if (conn.source.nodeID == sourceFilterUID &&
        conn.source.channelIndex == sourceFilterChannel &&
        conn.destination.nodeID == destFilterUID &&
        conn.destination.channelIndex == destFilterChannel) {
...
bool FilterGraph::canConnect(AudioProcessorGraph::NodeID sourceFilterUID,
  ...
  AudioProcessorGraph::Connection conn{{sourceFilterUID, sourceFilterChannel},
                                       {destFilterUID, destFilterChannel}};
  return graph.canConnect(conn);
}

bool FilterGraph::addConnection(AudioProcessorGraph::NodeID sourceFilterUID,
                                int sourceFilterChannel,
                                AudioProcessorGraph::NodeID destFilterUID,
...
void FilterGraph::removeConnection(AudioProcessorGraph::NodeID sourceFilterUID,
                                   int sourceFilterChannel,
                                   AudioProcessorGraph::NodeID destFilterUID,
                                   int destFilterChannel) {
...
void FilterGraph::clear(bool addAudioIn, bool addMidiIn, bool addAudioOut) {
  ...
  if (addAudioOut) {
    addFilter(internalFormat.getDescriptionFor(
                  InternalPluginFormat::audioOutputFilter),
              892.0f, 10.0f);
  }
...
//==============================================================================
static XmlElement *createNodeXml(AudioProcessorGraph::Node::Ptr node,
  ...
  e->setAttribute("uiLastX",
                  (int)node->properties.getWithDefault("uiLastX", 0.0));
  e->setAttribute("uiLastY",
                  (int)node->properties.getWithDefault("uiLastY", 0.0));
  e->setAttribute("windowOpen",
                  (bool)node->properties.getWithDefault("windowOpen", false));
  e->setAttribute("program", node->getProcessor()->getCurrentProgram());
  if (bypassable) {
    e->setAttribute("oscMIDIAddress",
                    oscManager.getMIDIProcessorAddress(bypassable));
...
void FilterGraph::createNodeFromXml(const XmlElement &xml,
  ...
  AudioProcessorGraph::Node::Ptr node(
      graph.addNode(std::move(instancePtr),
                    AudioProcessorGraph::NodeID(xml.getIntAttribute("uid"))));
...
void FilterGraph::restoreFromXml(const XmlElement &xml,
  ...
  forEachXmlChildElementWithTagName(xml, e2, "CONNECTION") {
    addConnection(
        AudioProcessorGraph::NodeID((uint32)e2->getIntAttribute("srcFilter")),
        e2->getIntAttribute("srcChannel"),
        AudioProcessorGraph::NodeID((uint32)e2->getIntAttribute("dstFilter")),
