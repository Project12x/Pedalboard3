...
StupidWindow::StupidWindow(const String &commandLine, bool startHidden)
    : DocumentWindow("Pedalboard 2", Colours::black,
...
StupidWindow::~StupidWindow() {
  ...
  PropertiesSingleton::getInstance().getUserSettings()->setValue(
      "WindowState", getWindowStateAsString());
  ...
  if (PropertiesSingleton::getInstance().getUserSettings()->getBoolValue(
    ...
    if (isVisible())
      ...
      FileBasedDocument::SaveResult result =
          mainPanel->saveIfNeededAndUserAgrees();
...
void StupidWindow::loadKeyMappings() {
  ...
  File mappingsFile =
      JuceHelperStuff::getAppDataFolder().getChildFile("AppMappings.xm");
      ...
      if (midiMappings) {
        for (i = 0; i < midiMappings->getNumChildElements(); ++i) {
          ...
          if (tempEl->hasTagName("MidiAppMapping")) {
            MidiAppMapping *newMapping =
                new MidiAppMapping(midiManager, tempEl);
...
void StupidWindow::saveKeyMappings() {
  ...
  File mappingsFile =
      JuceHelperStuff::getAppDataFolder().getChildFile("AppMappings.xm");
  XmlElement *mappingsXml =
      commandManager.getKeyMappings()->createXml(false).release();
