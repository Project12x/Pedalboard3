...
//==============================================================================
MappingsDialog::MappingsDialog(MidiMappingManager *manager,
                               OscMappingManager *manager2,
                               AudioProcessorGraph::Node *plugin,
                               Array<Mapping *> maps, PluginField *field)
    : mappingsList(0), addMidiButton(0), addOscButton(0), deleteButton(0),
      overrideMidiButton(0), oscMidiAddressLabel(0), oscMidiAddress(0),
      oscHintLabel(0), midiChannelLabel(0), midiChannelComboBox(0) {
  addAndMakeVisible(mappingsList = new ListBox(String(), this));
  ...
  addAndMakeVisible(overrideMidiButton =
                        new ToggleButton("overrideMidiButton"));
  overrideMidiButton->setButtonText(
      "Override plugin\'s default MIDI behaviour");
  overrideMidiButton->addListener(this);

  addAndMakeVisible(oscMidiAddressLabel =
                        new Label("oscMidiAddressLabel", "OSC MIDI Address:"));
  ...
  oscMidiAddress->setText(String());

  addAndMakeVisible(
      oscHintLabel = new Label(
          "oscHintLabel",
  ...
  addAndMakeVisible(midiChannelLabel =
                        new Label("midiChannelLabel", "MIDI Channel:"));
  ...
  mappingsList->setColour(
      ListBox::backgroundColourId,
      ColourScheme::getInstance().colours["Dialog Inner Background"]);

  overrideMidiButton->setToggleState(
      !pluginField->getMidiEnabledForNode(pluginNode), false);

  {
    BypassableInstance *proc =
        dynamic_cast<BypassableInstance *>(pluginNode->getProcessor());
...
void MappingsDialog::buttonClicked(Button *buttonThatWasClicked) {
  ...
  if (buttonThatWasClicked == addMidiButton) {
    ...
    MidiMapping *mapping = new MidiMapping(
        midiManager, pluginField->getFilterGraph(),
        pluginNode->nodeID.uid, // JUCE 8: NodeID.uid
        0, 0, false, midiChannelComboBox->getSelectedId() - 1, 0.0f, 1.0f);
    ...
    OscMapping *mapping =
        new OscMapping(oscManager, pluginField->getFilterGraph(),
                       pluginNode->nodeID.uid, // JUCE 8: NodeID.uid
                       0, "", 0);
...
void MappingsDialog::comboBoxChanged(ComboBox *comboBoxThatHasChanged) {
  ...
  if (comboBoxThatHasChanged == midiChannelComboBox) {
    //[UserComboBoxCode_midiChannelComboBox] -- add your combo box handling code
    //here..
...
//------------------------------------------------------------------------------
int MappingsDialog::getNumRows() { return mappings.size(); }

//------------------------------------------------------------------------------
void MappingsDialog::paintListBoxItem(int rowNumber, Graphics &g, int width,
                                      int height, bool rowIsSelected) {
  Colour highlight =
      ColourScheme::getInstance().colours["List Selected Colour"];

  if (rowIsSelected) {
    ColourGradient basil(highlight.brighter(0.4f), 0.0f, 0.0f,
                         highlight.darker(0.125f), 0.0f, (float)height, false);
...
//------------------------------------------------------------------------------
Component *
MappingsDialog::refreshComponentForRow(int rowNumber, bool isRowSelected,
  ...
  if (midiMapping) {
    retval = new MappingEntryMidi(
        this, rowNumber, midiMapping->getCc(), midiMapping->getLatched(),
    ...
    for (i = 0; i < numParams; ++i)
      ((MappingEntryMidi *)retval)
          ->addParameter(pluginNode->getProcessor()->getParameterName(i));
    ...
    retval = new MappingEntryOsc(this, rowNumber, oscMapping->getAddress(),
...
//------------------------------------------------------------------------------
void MappingsDialog::backgroundClicked() { mappingsList->deselectAllRows(); }
...
void MappingsDialog::midiCcReceived(int val) {
  if (midiLearnIndex > -1) {
    MidiMapping *mapping =
        dynamic_cast<MidiMapping *>(mappings[midiLearnIndex]);
...
//------------------------------------------------------------------------------
void MappingsDialog::handleAsyncUpdate() { updateListBox(); }

//------------------------------------------------------------------------------
void MappingsDialog::updateListBox() { mappingsList->updateContent(); }
