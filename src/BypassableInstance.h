...
public:
  ...
  const String getInputChannelName(int channelIndex) const {
    return plugin->getInputChannelName(channelIndex);
  };
  ///	Returns the indexed channel name.
  const String getOutputChannelName(int channelIndex) const {
    return plugin->getOutputChannelName(channelIndex);
  };
  ///	Returns true if the channel's a stereo pair.
  bool isInputChannelStereoPair(int index) const {
    return plugin->isInputChannelStereoPair(index);
  };
  ///	Returns true if the channel's a stereo pair.
  bool isOutputChannelStereoPair(int index) const {
    return plugin->isOutputChannelStereoPair(index);
  };
  // silenceInProducesSilenceOut() removed in JUCE 8 (now private in
  // AudioProcessor)
  ...
  void changeProgramName(int index, const String &newName) {
    plugin->changeProgramName(index, newName);
  };
