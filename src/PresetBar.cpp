...
PresetBar::PresetBar(PluginComponent *comp)
    : presetsComboBox(0), presetsLabel(0), importButton(0), saveButton(0) {
  ...
  addAndMakeVisible(
      importButton = new DrawableButton(
          "importButton", DrawableButton::ImageOnButtonBackground));
  ...
  auto openImage =
      loadSVGFromMemory(Vectors::openbutton_svg, Vectors::openbutton_svgSize);
...
PresetBar::loadSVGFromMemory(const void *dataToInitialiseFrom,
                             size_t sizeInBytes) {
