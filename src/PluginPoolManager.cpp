...
std::vector<PluginDescription> PluginPoolManager::extractPluginsFromPatch(const XmlElement* patchXml)
{
    ...
    for (auto* filterElem : patchXml->getChildWithTagNameIterator("FILTER"))
    {
        ...
        if (descElem)
        {
            ...
            desc.loadFromXml(*descElem);

            // Skip internal plugins (Audio I/O, etc.)
            if (desc.pluginFormatName != "Internal" && desc.pluginFormatName != "AudioUnit")
            {
