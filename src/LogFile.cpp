...
void LogFile::start()
{
    if (!isLogging)
    {
        ...
        tempstr << "Pedalboard3LogFile-" << String(timmy.getDayOfMonth()).paddedLeft('0', 2) << ".";
        ...
        logFile.reset(new FileOutputStream(File::getSpecialLocation(File::userApplicationDataDirectory)
                                               .getChildFile("Pedalboard3")
                                               .getChildFile(tempstr)));

        if (logFile->failedToOpen())
        {
            AlertWindow::showMessageBox(AlertWindow::WarningIcon, "LogFileError",
...
//------------------------------------------------------------------------------
const String& LogFile::getLogContents(const StringArray& eventTypes, Time& eventsSince)
{
    int i;
