...
PluginComponent::PluginComponent(AudioProcessorGraph::Node *n)
    : Component(),
      // plugin(p),
      editButton(0), mappingsButton(0), bypassButton(0), deleteButton(0),
      node(n), pluginWindow(0), beingDragged(false), dragX(0), dragY(0) {
  BypassableInstance *bypassable =
      dynamic_cast<BypassableInstance *>(node->getProcessor());
  ...
  if ((pluginName != "Audio Input") && (pluginName != "Midi Input") &&
    ...
    ScopedPointer<Drawable> closeUp(JuceHelperStuff::loadSVGFromMemory(
        Vectors::closefilterbutton_svg, Vectors::closefilterbutton_svgSize));
    ScopedPointer<Drawable> closeOver(JuceHelperStuff::loadSVGFromMemory(
        Vectors::closefilterbuttonover_svg,
        Vectors::closefilterbuttonover_svgSize));
    ScopedPointer<Drawable> closeDown(JuceHelperStuff::loadSVGFromMemory(
        Vectors::closefilterbuttondown_svg,
        Vectors::closefilterbuttondown_svgSize));
    ScopedPointer<Drawable> bypassOff(JuceHelperStuff::loadSVGFromMemory(
        Vectors::bypassbuttonoff_svg, Vectors::bypassbuttonoff_svgSize));
    ScopedPointer<Drawable> bypassOn(JuceHelperStuff::loadSVGFromMemory(
        Vectors::bypassbuttonon_svg, Vectors::bypassbuttonon_svgSize));
    ...
    deleteButton =
        new DrawableButton("DeleteFilterButton", DrawableButton::ImageRaw);
...
void PluginComponent::paint(Graphics &g) {
  ...
  g.drawRoundedRectangle(1.0f, 1.0f, (float)getWidth() - 2.0f,
                         (float)getHeight() - 2.0f, 5.0f, 1.0f);

  // Fill Component background.
  g.setColour(colours["Plugin Background"]);
  g.fillRoundedRectangle(2.0f, 2.0f, (float)getWidth() - 4.0f,
                         (float)getHeight() - 4.0f, 5.0f);

  // Fill Component outline and text background.
  g.setColour(colours["Plugin Border"]);
  g.drawRoundedRectangle(3.0f, 3.0f, (float)getWidth() - 6.0f,
                         (float)getHeight() - 6.0f, 5.0f, 4.0f);
...
//------------------------------------------------------------------------------
void PluginComponent::moved() { sendChangeMessage(); }
...
void PluginComponent::mouseDrag(const MouseEvent &e) {
  if (beingDragged) {
    ...
    // Viewport.
    Viewport *viewport = dynamic_cast<Viewport *>(
        getParentComponent()->getParentComponent()->getParentComponent());
...
//------------------------------------------------------------------------------
void PluginComponent::mouseUp(const MouseEvent &e) { beingDragged = false; }
...
void PluginComponent::openMappingsWindow() {
  ...
  PluginField *parent = dynamic_cast<PluginField *>(getParentComponent());
  MappingsDialog dlg(
      parent->getMidiManager(), parent->getOscManager(), node,
      /// @note JUCE 8: NodeID is now a struct, use .uid for integer value
...
void PluginComponent::determineSize(bool onlyUpdateWidth) {
  ...
  bool ignorePinNames =
      PropertiesSingleton::getInstance().getUserSettings()->getBoolValue(
  ...
  if (!proc) {
    ...
    if ((plugin->acceptsMidi() || (plugin->getNumInputChannels() > 0) ||
...
void PluginComponent::createPins() {
  ...
  if ((plugin->acceptsMidi() || (plugin->getNumInputChannels() > 0) ||
    ...
    pin = new PluginPinComponent(false, uid,
  ...
  if (plugin->producesMidi() || (plugin->getName() == "OSC Input")) {
    ...
    pin = new PluginPinComponent(true, uid,
                                 AudioProcessorGraph::midiChannelIndex, true);
    pinPos.setXY(getWidth() - 5, y);
    pin->setTopLeftPosition(pinPos.getX(), pinPos.getY());
    addAndMakeVisible(pin);
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
PluginPinComponent::PluginPinComponent(bool dir, uint32 id, int chan,
                                       bool param)
...
//------------------------------------------------------------------------------
PluginEditorWindow::EditorWrapper::~EditorWrapper() { deleteAllChildren(); }
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
PluginConnection::PluginConnection(PluginPinComponent *s, PluginPinComponent *d,
                                   bool allOutputs)
    : Component(), source(s), selected(false),
...
PluginConnection::~PluginConnection() {
  if (source) {
    PluginComponent *sourceComp =
        dynamic_cast<PluginComponent *>(source->getParentComponent());
    if (sourceComp)
      sourceComp->removeChangeListener(this);
  }
  if (destination) {
    PluginComponent *destComp =
        dynamic_cast<PluginComponent *>(destination->getParentComponent());
    if (destComp)
...
//------------------------------------------------------------------------------
void PluginConnection::changeListenerCallback(
    ChangeBroadcaster *changedObject) {
  ...
  if (source && destination) {
    ...
    sourcePoint =
        field->getLocalPoint(source->getParentComponent(), sourcePoint);
    destPoint =
        field->getLocalPoint(destination->getParentComponent(), destPoint);

    updateBounds(sourcePoint.getX(), sourcePoint.getY(), destPoint.getX(),
                 destPoint.getY());
  }
}
...
void PluginConnection::setDestination(PluginPinComponent *d) {
  ...
  destination = d;
  if (destination)
    ((PluginComponent *)destination->getParentComponent())
  ...
  if (source && destination) {
    ...
    if (destPoint.getX() > sourcePoint.getX())
      ...
      updateBounds(destPoint.getX(), destPoint.getY(), sourcePoint.getX(),
                   sourcePoint.getY());
  }
}
...
void PluginConnection::updateBounds(int sX, int sY, int dX, int dY) {
  ...
  Path tempPath;
  PathStrokeType drawnType(9.0f, PathStrokeType::mitered,
  ...
  tempPath.cubicTo(((float)width * 0.5f) + jmin(sX, dX), (float)sY,
                   ((float)width * 0.5f) + jmin(sX, dX), (float)dY, (float)dX,
                   (float)dY);
