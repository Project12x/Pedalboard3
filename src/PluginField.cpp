...
//------------------------------------------------------------------------------
PluginField::PluginField(FilterGraph *filterGraph, KnownPluginList *list,
                         ApplicationCommandManager *appManager)
    : signalPath(filterGraph), pluginList(list), midiManager(appManager),
      oscManager(appManager), draggingConnection(0), tempo(120.0),
  ...
  if (midiInputEnabled) {
    ...
    // And connect it up to the midi input.
    {
      ...
      for (i = 0; i < signalPath->getNumFilters(); ++i) {
        ...
        else if (signalPath->getNode(i)->getProcessor()->getName() ==
                 "Midi Interceptor") {
          midiInterceptor =
              signalPath->getNode(i)->nodeID; // JUCE 8: NodeID struct
          dynamic_cast<MidiInterceptor *>(
              signalPath->getNode(i)->getProcessor())
              ->setManager(&midiManager);
        }
      }
      signalPath->addConnection(
          midiInput, AudioProcessorGraph::midiChannelIndex, midiInterceptor,
...
PluginField::~PluginField() {
  ...
  for (i = (getNumChildComponents() - 1); i >= 0; --i) {
    PluginConnection *connection =
        dynamic_cast<PluginConnection *>(getChildComponent(i));
...
void PluginField::mouseDown(const MouseEvent &e) {
  if (e.getNumberOfClicks() == 2) {
    ...
    if (result > 0) {
      ...
      signalPath->addFilter(
          pluginList->getType(pluginList->getIndexChosenByMenu(result)),
...
void PluginField::timerCallback() {
  ...
  for (i = 0; i < getNumChildComponents(); ++i) {
    PluginComponent *comp =
        dynamic_cast<PluginComponent *>(getChildComponent(i));
...
bool PluginField::isInterestedInFileDrag(const StringArray &files) {
  ...
  for (i = 0; i < files.size(); ++i) {
    ...
    else if (files[i].endsWith(".wav") || files[i].endsWith(".aif") ||
             files[i].endsWith(".aiff") || files[i].endsWith(".ogg") ||
             files[i].endsWith(".flac") || files[i].endsWith(".wma")) {
      retval = true;
      break;
    }
  }
...
void PluginField::filesDropped(const StringArray &files, int x, int y) {
  ...
  if (pluginsInArray) {
    pluginList->scanAndAddDragAndDroppedFiles(
        AudioPluginFormatManagerSingleton::getInstance(), files, foundPlugins);
  ...
  if (soundsInArray) {
    for (i = 0; i < files.size(); ++i) {
      ...
      signalPath->addFilter(new FilePlayerProcessor(File(files[i])), (double)x,
                            (double)y);
...
void PluginField::enableAudioInput(bool val) {
  ...
  if (!val) {
    ...
    signalPath->addFilter(internalFormat.getDescriptionFor(
                              InternalPluginFormat::audioInputFilter),
                          10.0f, 10.0f);
...
void PluginField::enableMidiInput(bool val) {
  ...
  if (!val) {
    ...
    // Midi Interceptor filter.
    {
      // Delete filter.
      for (i = (signalPath->getNumFilters() - 1); i >= 0; --i) {
        if (signalPath->getNode(i)->getProcessor()->getName() ==
            "Midi Interceptor")
          deleteFilter(signalPath->getNode(i).get()); // JUCE 8: Node::Ptr
      }
    }
...
//------------------------------------------------------------------------------
void PluginField::setAutoMappingsWindow(bool val) { autoMappingsWindow = val; }

//------------------------------------------------------------------------------
void PluginField::setTempo(double val) { tempo = val; }
  ...
  if (index < signalPath->getNumFilters()) {
    ...
    if (node->getProcessor()->getName() != "Midi Interceptor") {
      ...
      if (LogFile::getInstance().getIsLogging()) {
        ...
        tempstr << "Added plugin to signal path: "
  ...
  if (draggingConnection) {
    ...
    if (p) {
      if (p->getParameterPin() ==
          draggingConnection->getParameterConnection()) {
    ...
    if (p) {
      if (!p->getDirection()) {
        ...
        if ((s->getParameterPin() && p->getParameterPin()) ||
          ...
          signalPath->addConnection(
              AudioProcessorGraph::NodeID(s->getUid()), s->getChannel(),
          ...
          if (p->getParameterPin()) {
            PluginComponent *pComp =
                dynamic_cast<PluginComponent *>(p->getParentComponent());
    ...
    if (c) {
      if (c->getSelected()) {
        ...
        if (c->getParameterConnection()) {
          ...
          String tempstr =
              signalPath->getNodeForId(AudioProcessorGraph::NodeID(sourceId))
                  ->getProcessor()
                  ->getName();
...
void PluginField::enableMidiForNode(AudioProcessorGraph::Node *node, bool val) {
  ...
  connection = (signalPath->getConnectionBetween(
                    midiInput->nodeID, AudioProcessorGraph::midiChannelIndex,
                    node->nodeID, AudioProcessorGraph::midiChannelIndex) != 0);
  if (val) {
    // If there's a connection, remove it.
    signalPath->removeConnection(
        midiInput->nodeID, AudioProcessorGraph::midiChannelIndex, node->nodeID,
        AudioProcessorGraph::midiChannelIndex);
  } else {
...
bool PluginField::getMidiEnabledForNode(AudioProcessorGraph::Node *node) const {
  ...
  if (!midiInput)
    ...
    return signalPath->getConnectionBetween(
               midiInput->nodeID, AudioProcessorGraph::midiChannelIndex,
               node->nodeID, AudioProcessorGraph::midiChannelIndex) != 0;
}
...
void PluginField::loadFromXml(XmlElement *patch) {
  ...
  // Add the audio/midi connections.
  {
    ...
    for (const auto &connection : signalPath->getConnections()) {
      ...
      for (j = 0; j < getNumChildComponents(); ++j) {
        PluginComponent *pluginComp =
            dynamic_cast<PluginComponent *>(getChildComponent(j));
  ...
  if (midiInputEnabled) {
    ...
    for (i = 0; i < signalPath->getNumFilters(); ++i) {
      interceptor = dynamic_cast<MidiInterceptor *>(
          signalPath->getNode(i)->getProcessor());
            ...
            for (i = 0; i < getNumChildComponents(); ++i) {
              ...
              if (comp) {
                if (comp->getNode()->nodeID.uid == uid) {
                  ...
                  addAndMakeVisible(
                      new PluginConnection(midiInput, paramInput));
...
Component *PluginField::getPinAt(const int x, const int y) {
  ...
  if (isVisible() && ((unsigned int)x) < (unsigned int)getWidth() &&
      ((unsigned int)y) < (unsigned int)getHeight() && hitTest(x, y)) {
    for (int i = getNumChildComponents(); --i >= 0;) {
      Component *const child = getChildComponent(i);

      if (!dynamic_cast<PluginConnection *>(child)) {
        Rectangle<int> updatedRect(child->getX() - 16, child->getY() - 16,
                                   child->getWidth() + 32,
                                   child->getHeight() + 32);
        ...
        if (updatedRect.contains(pos)) {
          ...
          Component *const c = child->getComponentAt(
              pos.getX() - child->getX(), pos.getY() - child->getY());
...
void PluginField::connectAll(PluginConnection *connection) {
  PluginComponent *source = dynamic_cast<PluginComponent *>(
      connection->getSource()->getParentComponent());
  PluginComponent *dest = dynamic_cast<PluginComponent *>(
      connection->getDestination()->getParentComponent());
  ...
  if (source && dest) {
    ...
    for (; (left < numSources) && (right < numDests); ++left, ++right) {
      ...
      signalPath->addConnection(
          AudioProcessorGraph::NodeID(sourcePin->getUid()),
