...
//------------------------------------------------------------------------------
void LevelProcessor::fillInPluginDescription(
    PluginDescription &description) const {
...
//------------------------------------------------------------------------------
void FilePlayerProcessor::fillInPluginDescription(
    PluginDescription &description) const {
...
void FilePlayerProcessor::setParameter(int parameterIndex, float newValue) {
  ...
  case Looping:
    ...
    break;
  case ReadPosition:
    transportSource.setPosition(newValue *
...
//------------------------------------------------------------------------------
void FilePlayerProcessor::setStateInformation(const void *data,
                                              int sizeInBytes) {
  std::unique_ptr<XmlElement> xmlState(getXmlFromBinary(data, sizeInBytes));
...
//------------------------------------------------------------------------------
void OutputToggleProcessor::fillInPluginDescription(
    PluginDescription &description) const {
...
//------------------------------------------------------------------------------
void OutputToggleProcessor::setStateInformation(const void *data,
                                                int sizeInBytes) {
  std::unique_ptr<XmlElement> xmlState(getXmlFromBinary(data, sizeInBytes));
...
//------------------------------------------------------------------------------
void VuMeterProcessor::fillInPluginDescription(
    PluginDescription &description) const {
...
RecorderProcessor::RecorderProcessor()
    : threadWriter(0),
      thumbnail(512, AudioFormatManagerSingleton::getInstance(),
                AudioThumbnailCacheSingleton::getInstance()),
      recording(false), stopRecording(false), syncToMainTransport(false),
      currentRate(44100.0) {
  setPlayConfigDetails(2, 0, 0, 0);
...
void RecorderProcessor::setFile(const File &phil) {
  ...
  if (soundFile != File()) {
    ...
    writer =
        wavFormat.createWriterFor(philStream, currentRate, 2, 16, metadata, 0);
    if (!writer) {
      delete philStream;
      soundFile = File();
      threadWriter = 0;
...
//------------------------------------------------------------------------------
void RecorderProcessor::cacheFile(const File &phil) { soundFile = phil; }
...
//------------------------------------------------------------------------------
void RecorderProcessor::fillInPluginDescription(
    PluginDescription &description) const {
...
MetronomeProcessor::MetronomeProcessor()
    : playing(false), syncToMainTransport(false), numerator(4), denominator(4),
      sineX0(1.0f), sineX1(0.0f), sineEnv(0.0f), clickCount(0.0f),
      clickDec(0.0f), measureCount(0), isAccent(true) {
...
//------------------------------------------------------------------------------
void MetronomeProcessor::fillInPluginDescription(
    PluginDescription &description) const {
  ...
  for (i = 0; i < numSamples; ++i) {
    if (playing) {
      ...
      if (clickCount <= 0.0f) {
        ...
        // The accent.
        if (!measureCount) {
          sineCoeff = 2.0f * sinf(3.1415926535897932384626433832795f * 880.0f *
                                  2.0f / 44100.0f);
          ...
          sineCoeff = 2.0f * sinf(3.1415926535897932384626433832795f * 440.0f *
                                  2.0f / 44100.0f);
          isAccent = false;
...
//------------------------------------------------------------------------------
void MetronomeProcessor::setStateInformation(const void *data,
                                             int sizeInBytes) {
  std::unique_ptr<XmlElement> xmlState(getXmlFromBinary(data, sizeInBytes));
...
LooperProcessor::LooperProcessor()
    : playing(false), stopPlaying(false), recording(false),
      stopRecording(false), syncToMainTransport(false), stopAfterBar(false),
      autoPlay(true), inputLevel(1.0f), loopLevel(1.0f), threadWriter(0),
      thumbnail(512, AudioFormatManagerSingleton::getInstance(),
                AudioThumbnailCacheSingleton::getInstance()),
      numerator(4), denominator(4), clickCount(0.0f), clickDec(0.0f),
      measureCount(0), currentRate(44100.0), justPaused(false), loopLength(0),
      loopPos(0), loopIndex(0), deleteLastBuffer(false), tempBufferWrite(0),
      fadeOutCount(-1), fadeInCount(0), autoPlayFade(1.0f), fileReader(0),
      fileReaderPos(0), fileReaderBufIndex(0), newFileLoaded(false),
      inputAudio(2, 2560) {
  int i;
...
void LooperProcessor::setFile(const File &phil) {
  ...
  if (soundFile != File()) {
    if (fileReader)
      delete fileReader;
    fileReader =
        AudioFormatManagerSingleton::getInstance().createReaderFor(soundFile);
    fileReaderPos = 0;
  ...
  /*if(soundFile.existsAsFile())
  {
          if(!soundFile.deleteFile())
          {
                  AlertWindow::showMessageBox(AlertWindow::WarningIcon,
                                                                          "Could
  not delete existing file", "Have you got the file open elsewhere? (e.g. in
  another File Player)");
...
double LooperProcessor::getReadPosition() const {
  if (loopLength > 0)
    return (double)(((double)loopIndex * (double)LoopBufferSize) +
                    (double)loopPos) /
           (double)loopLength;
  else
    return 0.0;
  ...
  while (playing) {
    ...
    if (!abortCounter) {
      AlertWindow::showMessageBox(
          AlertWindow::WarningIcon, "Looper Error",
...
int LooperProcessor::useTimeSlice() {
  ...
  if (recording) {
    ...
    fileReader->read(loopBuffer[fileReaderBufIndex], 0, LoopBufferSize,
...
//------------------------------------------------------------------------------
void LooperProcessor::fillInPluginDescription(
    PluginDescription &description) const {
    ...
    if (loopIndex < loopBuffer.size()) {
      ...
      if (i < samplesToRecord) {
        ...
        if (loopIndex >= loopBuffer.size()) {
          ...
          loopBuffer[loopIndex]->copyFrom(0, 0, buffer, 0, i,
                                          (samplesToRecord - i));
          loopBuffer[loopIndex]->copyFrom(1, 0, buffer, 1, i,
                                          (samplesToRecord - i));
  ...
  if (playing && (loopLength > 0)) {
    ...
    for (j = 0; j < numSamples; ++j, --fadeInCount) {
      if (fadeInCount <= (FadeBufferSize - 1)) {
        ...
        data[0][j] += fadeInBuffer[0][FadeBufferSize - 1 - fadeInCount] *
                      tempf * loopLevel;
        data[1][j] += fadeInBuffer[1][FadeBufferSize - 1 - fadeInCount] *
                      tempf * loopLevel;
    ...
    if (autoPlayFade < 1.0f) {
      buffer.addFromWithRamp(0, 0,
                             loopBuffer[loopIndex]->getWritePointer(0, loopPos),
                             i, 0.0f, loopLevel);
      buffer.addFromWithRamp(1, 0,
                             loopBuffer[loopIndex]->getWritePointer(1, loopPos),
                             i, 0.0f, loopLevel);
      autoPlayFade = 1.0f;
    } else {
    ...
    if (i < numSamples) {
      ...
      buffer.addFrom(0, i, *(loopBuffer[loopIndex]), 0, loopPos,
                     (numSamples - i), loopLevel);
      buffer.addFrom(1, i, *(loopBuffer[loopIndex]), 1, loopPos,
                     (numSamples - i), loopLevel);
...
void LooperProcessor::setStateInformation(const void *data, int sizeInBytes) {
  ...
  if (xmlState != 0) {
    if (xmlState->hasTagName("Pedalboard2LooperSettings")) {
      ...
      syncToMainTransport =
          xmlState->getBoolAttribute("syncToMainTransport", false);
