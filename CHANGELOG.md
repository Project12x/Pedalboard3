...
- **Playwright Mockup Reference QA** — Added local Playwright/React/Babel dev dependencies for rendering design handoff HTML through Edge, with reference evidence in `documentation/qa/2026-06-09-mockup-reference`.
...
- **Stage Mode Layout Contract** — Added deterministic `StageLayout` metrics and Catch2 coverage for responsive Stage Mode typography, controls, tuner regions, and long patch-name elision.
- **Core UI Token Audit Gate** — Added source-level UI regression coverage preventing hardcoded white/hex colour literals and ad-hoc `FontOptions().withHeight()` usage from returning to the audited main shell, graph canvas, connection, and Stage Mode files.
- **P0 Visual QA Evidence** — Added `documentation/qa/2026-06-01-p0-ui-polish` with current theme, dense graph, Stage Mode, patch-switch, and dialog screenshots after the P0 polish closure pass.

### Changed

- **Tuner Quick-Read Layout** — Enlarged the tuner node and prioritized its note readout, coarse-deviation strip, and primary tuning meter. The secondary pitch-history, status, and reference-response panels are no longer drawn, trading historical detail for a clearer immediate tuning surface during live setup.
- **JUCE 9 Framework Migration** — Updated the vendored JUCE dependency from 8.0.12 to 9.0.0 and migrated the host UI to its supported editor, SVG, font-resource, and shaped-text measurement APIs. This keeps current Windows builds compatible with the maintained framework release, prevents the bundled static Space Grotesk font from failing JUCE 9's memory-font path, and improves fidelity for complex SVG assets. The parent project now enables C as well as C++ because JUCE 9 builds bundled C sources.
...
- **Secondary Mockup Harvest Browser Polish** — NAM browser, IR browser, plugin search, and IR loader surfaces now carry more of the mockup's native aesthetic language: faceplate headers, list-card selection glow, glyph tiles, stronger empty states, footer actions, and clearer status feedback while preserving existing workflows.
- **Secondary Mockup Harvest Aesthetic Polish** — Plugin search now has a mockup-informed instrument-panel title signal, framed search/status header, segmented category controls, and status footer copy; NAM/IR browser detail panels now use backed key-value rows that preserve all existing fields while improving scan hierarchy.
- **Main Graph Control Polish** — Shared TextButton and linear slider rendering now use mockup-informed depth, sheen, active/hover states, and theme-aware accents; the main graph adds a persisted `Options > Graph Grid` selector for `Dots`, `Lines`, and `Off`.
- **Optional Node Parameter Controls** — Wrapped plugin nodes can show up to three compact, drag-adjustable parameter strips in the main graph body, with a persisted `Options > Node Parameter Controls` toggle to disable the extra density.
- **Stage Grid Bank Rail** — Stage Grid mode now uses mockup-aligned Bank A/B/C labels, clickable visible bank selectors for larger sets, and stronger LED-style Live badges while preserving tile-based patch switching.
- **Stage View Schema Polish** — Stage Grid and Setlist views no longer reserve the large tuner content strip; the full tuner remains available through the dedicated Tune view while Grid/Setlist keep their patch content density closer to the mockup.
- **IR Loader Chassis Polish** — Standalone IR Loader controls now use the mockup-informed amp faceplate palette, theme-reactive buttons/sliders, semantic typography, dual-slot LED state, cabinet glyph, and loaded/empty footer status while preserving Load, Browse, Clear, Blend, Mix, and filter behavior.
- **IR Browser Preview Polish** — IR Browser now uses a mockup-informed selected preview card with cabinet glyph, READY/MISSING/EMPTY state chips, compact IR metadata, backed detail rows, and Load IR enablement tied to an existing selected file.
- **Local NAM Browser Preview Polish** — Local NAM browsing now has a mockup-informed selected-model preview card with architecture glyph, READY/MISSING/EMPTY state chips, model-type/tone chips, richer metadata search, formatted model details, and Load/Delete enablement tied to an existing `.nam` file.
...
- **Stage Mode Responsiveness** — Stage Mode now derives header/footer/tuner areas, live-performance button sizes, meter geometry, tuner bar sizing, and display font heights from viewport-aware metrics instead of fixed constants.
- **Core UI Token Usage** — Replaced remaining audited hardcoded white/hex colour and one-off font literals in `MainPanel`, `PluginComponent`, `PluginConnection`, and `StageView` with semantic colour roles, derived contrast, and `FontManager` tiers.
...
- **NAM/IR Browser Detail State** — Populated NAM and IR browsers now select and preview the first available row after scans and filters instead of opening with blank details and enabled load actions.
- **NAM Browser High-Scale Actions** — NAM Load Model, Delete Model, and Close are now grouped in the footer action row so primary actions remain reachable in the 200% narrow scaled dialog matrix.
...
- **NAM Dual-IR Heap Allocation on Audio Thread** — `processBlock` created a local `AudioBuffer` and called `makeCopyOf()` (heap alloc) every block when dual-IR was active. Moved `ir2Buffer` to a class member pre-allocated in `prepareToPlay()`, replaced with `copyFrom()` (zero-alloc).
- **NAM IIR Coefficient Heap Allocation on Audio Thread** — `updateIRFilters()` used JUCE factory methods (`makeHighPass`/`makeLowPass`) which allocate `ReferenceCountedObject` on the heap. Replaced with inline 2nd-order Butterworth biquad computation via bilinear transform, writing directly to existing coefficient arrays.
- **Debug cerr Noise** — Removed 32 leftover `std::cerr` debug statements from `PluginComponent::buttonClicked()` and `PluginEditorWindow` lifecycle methods. These blocked on I/O and cluttered stderr output.
...
- **Canvas Scrolling & Pan** — Canvas was sized exactly to the viewport, preventing scroll, pan, and scrollbar visibility in all directions. Added 1000px padding in `MainPanel::resized()` and `PluginField::fitToScreen()` so scrollbars appear and panning works immediately without needing to expand the canvas first.
- **Zoom-to-Cursor** — Zoom was not anchoring to the mouse cursor position. Two bugs: (1) `setTransform()` was called before `getViewPosition()`, corrupting the old scroll position; (2) mouse position was in component-local (pre-transform) coords instead of viewport-relative coords. Fixed by capturing positions first, using screen coordinates, then applying the transform. Canvas also auto-expands during zoom to prevent viewport clamping.
- **Browser Gradient Banding** — Restored original single-pass opaque gradients in Online, Local/IR, and IR browser `paint()` methods. Removed procedural noise texture overlays that caused visible banding. Boosted dot-grid pattern visibility (0.05f alpha, 2px dots).
